...
import { Loader2, Upload, X, ImageIcon, Plus, Trash2, Pencil, Check } from "lucide-react";
      ...
      <div className="max-w-4xl">
        ...
        <div className="mb-8 pb-4 border-b border-border flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">Add, edit, or remove product categories.</p>
          </div>
          <button
            onClick={() => { setShowAddForm(true); setNameEditId(null); setImageEditId(null); }}
            className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-bold uppercase tracking-widest text-xs px-5 py-3 hover:bg-primary/90 transition-colors"
          >
                ...
                <div className="flex items-start gap-4 p-5">
                  ...
                  <div className="flex-1 min-w-0">
                      ...
                      {nameEditId === category.id ? (
                        <>
                          <input
                            ...
                            onKeyDown={(e) => { if (e.key === "Enter") handleSaveName(category.id); if (e.key === "Escape") setNameEditId(null); }}
                            className="bg-background border border-primary px-2 py-1 text-sm font-bold focus:outline-none flex-1 max-w-xs"
                          />
                          <button onClick={() => handleSaveName(category.id)} className="p-1 text-primary hover:text-primary/80">
                            <Check className="w-4 h-4" />
                          </button>
                          <button onClick={() => setNameEditId(null)} className="p-1 text-muted-foreground hover:text-foreground">
                            <X className="w-4 h-4" />
                          </button>
                        </>
                      ) : (
                        <>
                          ...
                          <button
                            onClick={() => { setNameEditId(category.id); setEditName(category.name); setImageEditId(null); }}
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      <button
                        onClick={() => {
                          setImageEditId(imageEditId === category.id ? null : category.id);
                      ...
                      {confirmDeleteId === category.id ? (
                        ...
                        <button
                          onClick={() => { setConfirmDeleteId(category.id); setImageEditId(null); setNameEditId(null); }}
